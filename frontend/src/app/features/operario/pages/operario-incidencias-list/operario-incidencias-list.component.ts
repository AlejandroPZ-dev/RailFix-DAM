...
@Component({
  selector: 'app-operario-incidencias-list',
  templateUrl: './operario-incidencias-list.component.html',
  styleUrls: ['./operario-incidencias-list.component.css'],
  standalone: false
})
export class OperarioIncidenciasListComponent implements OnInit, OnDestroy {
  ...
  get filteredIncidencias(): Incidencia[] {
    return this.incidencias.filter((incidencia) => {
      ...
      const matchesLinea = !this.filters.lineaId
        || incidencia.lineaId === Number(this.filters.lineaId);
